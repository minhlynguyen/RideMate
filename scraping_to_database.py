...
def stations_to_db(text):
    ...
    for station in stations:
        ...
        vals = (station.get('number'), station.get('address'), int(station.get('banking')),
                ...
                db_update)
        engine.execute(
            "insert into station values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", vals)
    return
...
def stations_availability_to_db(text):
    ...
    for station in stations:
        ...
        vals = (station.get('number'), station.get('available_bikes'),
                ...
                db_update)
        engine.execute(
            "insert into availability values(%s,%s,%s,%s,%s,%s)", vals)
    return
...
#     print(weather_dict)
    ...
    for station in weather_station:
        ...
        vals_current = (station, last_update, temperature,
                        weathercode, windspeed, db_update)
        engine.execute(
            "insert into weather_current values(%s,%s,%s,%s,%s,%s)", vals_current)
    return
